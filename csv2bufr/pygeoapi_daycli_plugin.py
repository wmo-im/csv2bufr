...
def update_template(template, metadata):
    ...
    template['header']['bufrHeaderCentre'] = f"const:{metadata['data_identification']['originating_centre']}"
    template['header']['bufrHeaderSubCentre'] = f"const:{metadata['data_identification']['originating_subcentre']}"
    template['data']['wsi_series'] = f"const:{wsi_series}"
    template['data']['wsi_issuer'] = f"const:{wsi_issuer}"
    template['data']['wsi_issue'] = f"const:{wsi_issue}"
    template['data']['wsi_local'] = f"const:{wsi_local}"
    template['data']['#1#latitude'] = f"const:{metadata['station_location']['latitude']}"
    template['data']['#1#longitude'] = f"const:{metadata['station_location']['longitude']}"
    template['data']['#1#heightOfStationGroundAboveMeanSeaLevel'] = f"const:{metadata['station_location']['station_height_above_sea_level']}"
    template['data']['#1#methodUsedToCalculateTheAverageDailyTemperature'] = f"const:{metadata['observing_practices']['method_of_calculating_mean_temperature']}"
    template['data']['#1#timePeriod'] = f"const:{metadata['observing_practices']['total_precipitation_day_offset']}"
    hour, minute, second = metadata['observing_practices']['total_precipitation_start_time'].split(":")
    template['data']['#1#hour'] = f"const:{hour}"
    template['data']['#1#minute'] = f"const:{minute}"
    template['data']['#1#second'] = f"const:{second}"
    template['data']['#2#timePeriod'] = f"const:{metadata['observing_practices']['fresh_snow_depth_day_offset']}"
    hour, minute, second = metadata['observing_practices']['fresh_snow_depth_start_time'].split(":")
    template['data']['#2#hour'] = f"const:{hour}"
    template['data']['#2#minute'] = f"const:{minute}"
    template['data']['#2#second'] = f"const:{second}"
    template['data']['#3#timePeriod'] = f"const:{metadata['observing_practices']['total_snow_depth_day_offset']}"
    ...
    template['data']['#4#minute'] = f"const:{minute}"
    template['data']['#4#second'] = f"const:{second}"
    template['data']['#5#timePeriod'] = f"const:{metadata['observing_practices']['minimum_temperature_day_offset']}"
    hour, minute, second = metadata['observing_practices']['minimum_temperature_start_time'].split(":")
    template['data']['#5#hour'] = f"const:{hour}"
    template['data']['#5#minute'] = f"const:{minute}"
    template['data']['#5#second'] = f"const:{second}"
    template['data']['#6#timePeriod'] = f"const:{metadata['observing_practices']['mean_temperature_day_offset']}"
